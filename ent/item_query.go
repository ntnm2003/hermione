...
func (iq *ItemQuery) QueryVendors() *VendorQuery {
	query := &VendorQuery{config: iq.config}
...
func (iq *ItemQuery) Clone() *ItemQuery {
	...
	return &ItemQuery{
		...
		withVendors: iq.withVendors.Clone(),
...
func (iq *ItemQuery) WithVendors(opts ...func(*VendorQuery)) *ItemQuery {
	query := &VendorQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
	iq.withVendors = query
	return iq
}
...
func (iq *ItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Item, error) {
	...
	if query := iq.withVendors; query != nil {
		if err := iq.loadVendors(ctx, query, nodes, nil,
			func(n *Item, e *Vendor) { n.Edges.Vendors = e }); err != nil {
			return nil, err
		}
	}
...
func (iq *ItemQuery) loadVendors(ctx context.Context, query *VendorQuery, nodes []*Item, init func(*Item), assign func(*Item, *Vendor)) error {
